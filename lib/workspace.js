...
function getIndex(device, callback) {
    ...
    projectJSONCombiner.getProjectJSON(path.dirname(this.projectJSONPath), fs, function(err, jsonObj) {
        ...
        walkFolder(me.assetsPath, function walk_callback_assets(files) {
        ...
        if(me.settings && me.settings.config && me.settings.config.paths && me.settings.config.paths.output) {
            ...
            if (output.root) { 
                taskCount += 1;
                walkFolder(path.join(me.path, output.root), function walk_callback_scripts(files) {
                    processFolder(index, elimaneteUnnecessaryScriptsFiles(files), "script", done);
                });
            }
            if(output.include && output.include.length) {
                taskCount += output.include.length;
                walkFolder(me.scriptsPath, function walk_callback_scripts(files) {
                    output.include.forEach( (includePath) => {
                        processFolder(index, elimaneteUnnecessaryScriptsFiles(files, new RegExp(path.join(me.scriptsPath, includePath))), "script", done);
                    })
                });
            }
        } else {
        ...
        walkFolder(me.fontPath, function walk_callback_fonts(files) {
            processFolder(index, files, "font", done);
        });

        var otherMapping = [{
            path: path.join(me.configPath, "defaults.xml"),
            scheme: "config",
        ...
        function setBinaryHashes() {
            const playerFolder = path.join(globalModulesPath, "smartface", "bin");
            var playerPath;
            var playerName;
            var fileHashes = {};
            if (device.os === "iOS") {
            ...
            playerPath = path.join(playerFolder, playerName);
        ...
        function updateScriptsPath(index) {
            ...
            me.scriptsPath = path.join(me.path, iOSScripts);
            me.settingsPath = path.join(me.scriptsPath, 'settings.json');
        }
 ...
 * @property {string} fontPath - Path of the scripts folder relative to workspace. Defaults to path.join(options.path, "config", "Fonts")
...
function Workspace(options) {
    ...
    /** @type {string} */
    this.settingsPath = path.join(this.scriptsPath, "settings.json");
    ...
    /** @type {string} */
    this.fontPath = path.join(this.path, path.join("config", "Fonts"));
...
function elimaneteUnnecessaryScriptsFiles(files, userRegExp) {
    const resultFiles = {};
    const jsRegExp = /\.js$|\.json$|\.jsx$/;
